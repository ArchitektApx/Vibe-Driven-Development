...
missing. The rest of the asking sits outside a Loop: `vdd-start-loop` runs the
whole interview, `vdd-setup` asks about a leftover `LOOP.md`, and the
PR-Author asks its deferred question after the Loop has ended, in a session
the user is sitting in. Asking at Sign-off would also ask the same question
twice, once per Loop, and the answer is one preference about how the user
wants to work.
